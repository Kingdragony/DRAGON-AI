  ...
  if (autobio === 'TRUE') {
    setInterval(() => {
      ...
      client.updateProfileStatus(
        ` ${date.toLocaleString('en-US', { timeZone: 'Africa/Nairobi' })}  ⏰️  ${date.toLocaleString('en-US', { weekday: 'long', timeZone: 'Africa/Nairobi'})}. ᴅʀᴀɢᴏɴ-ᴀɪ ɪs ᴏɴʟɪɴᴇ 🚀.`
      );
    }, 10 * 1000);
  }
...
if (!client.public && !mek.key.fromMe && chatUpdate.type === "notify") return;
      ...
      const raven = require("../action/arlodragon");
 ...
 client.ev.on('call', async (callData) => {
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      client.sendMessage(client.user.id, { text: `ᴅʀᴀɢᴏɴ-ᴀɪ ɪs ᴏɴʟɪɴᴇ🎭 ` });
    }
  });
